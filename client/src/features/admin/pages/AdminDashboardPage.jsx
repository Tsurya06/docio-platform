...
/**
 * AdminDashboardPage — Step 12 landing for role=admin. Renders the aggregation payload
 * from `/admin/dashboard` as KPI cards + a small "doctor-pending" alert that links to the
 * forthcoming approval list in Step 15. Numbers degrade gracefully on missing fields —
 * aggregation streams (`byRole`, `byStatus`) come back as `{ key: count }` maps; an
 * absent key just means "zero in that bucket today", which we treat as 0.
 *
 * We avoid nested totals on the client — the server returns each total directly when
 * available (the only one we sum is `appointments.total`, where the server does the sum,
 * not us). Per-role and per-status breakdowns are display-only.
 */
export default function AdminDashboardPage() {
  useTitle('Admin dashboard');
  const { data, isLoading, error } = useGetDashboardQuery();

  const agg = data?.data?.dashboard ?? {};
