...
import type { DoctorProfileUpdateInput, AvailabilityInput } from '../schemas.js';
...
export default function DoctorProfilePage() {
  useTitle('My profile');
  const [profileForm] = Form.useForm<DoctorProfileUpdateInput>();
  const [availForm] = Form.useForm<any>();
  ...
  const submitProfile = async (values: DoctorProfileUpdateInput) => {
    try {
      await updateProfile({
  ...
  const submitAvailability = async (values: any) => {
    try {
